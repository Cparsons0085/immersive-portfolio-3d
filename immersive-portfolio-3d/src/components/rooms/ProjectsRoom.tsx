...
export default function ProjectsRoom() {
  ...
  const carouselItems = filteredProjects.map(project => ({
    id: project.id,
    title: project.title,
    image: `https://picsum.photos/400/300?random=${project.id}`,
      ...
      >
        {viewMode === 'carousel' ? (
          ...
          <div className="min-h-[600px]">
            ...
            >
              <div className="glass border border-cyan-400/30 rounded-lg p-6 max-w-2xl mx-auto neon-glow-cyan">
                <h3 className="text-xl font-bold text-cyan-400 mb-2">
                  Interactive 3D Project Showcase
                </h3>
                <p className="text-white/70 text-sm">
                  Hover over projects to explore • Click to focus • Use arrow controls to navigate
                </p>
                <div className="flex justify-center space-x-4 mt-4 text-sm font-mono">
                  <span className="text-cyan-400">🖱️ Click to select</span>
                  <span className="text-magenta-400">⏸️ Auto-rotate</span>
                  <span className="text-green-400">🔗 Direct links</span>
                </div>
              </div>
            </motion.div>
          </div>
