...
export default function HeroRoom({ onNavigate, onTerminalComplete }: HeroRoomProps) {
  ...
  const terminalLines = [
    '$ whoami',
    'cristy@cyberpunk-portfolio:~$ ',
    '',
    'Welcome to the Neon Portfolio Experience',
    '',
    'Name: Cristy Parsons',
    'Role: Software Engineer & Automation Specialist',
    'Location: San Francisco, CA',
    'Specialties: Web Development | 3D Experiences | Automation | GIS',
    '',
    'Available Systems:',
    '├── SKILLS Laboratory     [ONLINE]',
    '├── PROJECTS Showcase     [ACTIVE]',
    '├── CONTACT Interface     [READY]',
    '└── AI Research Lab       [SECURE]',
    '',
    'System initialization complete.',
    'Ready for exploration...',
    '',
    '$ explore --elevator',
    'Elevator system activated ✓',
    'cristy@cyberpunk-portfolio:~$ _'
  ]
    ...
    if (currentLine < terminalLines.length) {
      ...
      const typeInterval = setInterval(() => {
        if (charIndex <= line.length) {
          ...
          }, currentLine === 0 || currentLine === 19 ? 1000 : 100) // Longer pause for commands
        }
      }, currentLine === 0 || currentLine === 19 ? 80 : 25) // Slower typing for commands
      ...
      // Show elevator button after terminal completes
      setTimeout(() => {
        setShowElevatorButton(true)
      }, 800)
      ...
      }, 1500) // Delay navigation appearance
    }
    ...
    <div className="min-h-screen flex items-center justify-center relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 w-full max-w-7xl px-6 items-center">
          ...
          <div className="glass border-neon-gradient rounded-xl overflow-hidden neon-glow-cyan shadow-2xl">
            ...
            <div className="bg-black/60 px-6 py-4 border-b border-cyan-400/30 flex items-center space-x-3">
              ...
              <div className="flex items-center space-x-2 text-cyan-400">
                <FiTerminal size={18} />
                <span className="text-sm font-mono">cyberpunk-portfolio-terminal</span>
              </div>
            ...
            {/* Terminal Content */}
            <div className="bg-black/80 p-8 font-mono text-sm h-[420px] overflow-y-auto">
        ...
        >
          ...
          <div className="glass rounded-2xl p-8 border-neon-gradient relative overflow-hidden neon-glow-magenta">
            ...
            <div className="relative z-10 text-center space-y-6">
              ...
              <div className="space-y-4">
                <h1 className="text-4xl font-bold neon-text-cyan">
                  Cristy Parsons
                </h1>
                <div className="text-xl text-magenta-400 font-semibold">
                ...
                {personalInfo.map((info, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="flex items-center space-x-3 p-3 bg-black/20 rounded-lg border border-white/10 hover:border-cyan-400/30 transition-all duration-300"
                    >
          ...
          {/* Enter Elevator Button - CRITICAL: High z-index and proper positioning */}
          {showElevatorButton && (
            <motion.div
              initial={{ opacity: 0, y: 30, scale: 0.8 }}
              animate={{ opacity: 1, y: 0, scale: 1 }}
              transition={{ type: "spring", duration: 0.8 }}
              className="text-center relative z-50" // CRITICAL: z-50 to ensure it's above navigation
            >
              <motion.button
                onClick={() => onNavigate(1)}
                className="group relative inline-flex items-center space-x-4 px-10 py-5 bg-gradient-to-r from-cyan-500/20 to-magenta-500/20 border-2 border-cyan-400 rounded-full font-mono font-bold text-cyan-400 transition-all duration-300 hover:border-magenta-400 hover:text-magenta-400 neon-glow-cyan hover:neon-glow-magenta shadow-2xl"
                whileHover={{
                  scale: 1.05,
                  boxShadow: '0 0 50px rgba(0, 255, 255, 0.8)'
                }}
                whileTap={{ scale: 0.95 }}
                style={{ zIndex: 100 }} // Additional z-index protection
              >
                <FiArrowUp size={24} />
                <span className="text-lg">ENTER ELEVATOR</span>

                {/* Animated Background Gradient */}
                <motion.div
                  className="absolute inset-0 bg-gradient-to-r from-cyan-400/10 to-magenta-400/10 rounded-full"
                  animate={{
                    opacity: [0.3, 0.8, 0.3],
                    scale: [1, 1.02, 1],
                  }}
